...
execute(message) {
    ...
    if (!queue.playing) {
      ...
      .setAuthor(`${message.author.username} resumed the music!`, "https://cdn.discordapp.com/attachments/778954384667181076/864948750392098846/Nightcorer-spin-nuovo.gif")
      //send the approve
      return queue.textChannel.send(playembed).catch(console.error);
    }
    //if its not paused return error
    return  attentionembed(message, "The music is not paused!").catch(console.error);
  }
};
